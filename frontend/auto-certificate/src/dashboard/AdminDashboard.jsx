...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      setUsersError(null);
      try {
        const res  = await fetch("http://localhost:5001/auth/login/getAllUser");
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      try {
        const res  = await fetch("http://localhost:5001/auth/login/getAllProgress");
        if (!res.ok) throw new Error(`HTTP error: ${res.status}`);
        const json = await res.json();
  ...
  const updateLog = async (id, newStatus) => {
    try {
      const res = await fetch(`http://localhost:5001/progress/${id}/status`, {
        method: "PATCH",
